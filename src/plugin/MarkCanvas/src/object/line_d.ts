 ...
 * @Descripttion: 
 * @version: 
 * @FilePath: /mark-canvas-demo/src/plugin/MarkCanvas/src/object/line_d.ts
 */
 ...
 * 标注对象 POLYGON
 */
export default class MarkLine_DObject extends MarkObject {
  // 最后按下
  ...
  constructor(box: MarkCanvas) {
    ...
    // 标注类型
    this.type = MarkObjectType.LINE_D;
  ...
  async complete() {
    ...
    } else if (this.box.currentDrawingType == MarkObjectType.LINE_D) {
      obj = new MarkLine_DObject(this.box);
    }
  ...
  render() {
    ...
    this.pointList.forEach((point, index) => {
      ...
      // 非编辑模式下只显示第一个顶点
      if (this.status !== "done" || (index === 0 && this.box.selectObject == this)) {
    ...
    if (this.status == "done" || this.status == "edit") {
      ...
      if (this.pointList.length >= 3) {
        let offset = {
          x: point.x - this.pointList[0].x,
          y: point.y - this.pointList[0].y,
        };
        if (Math.abs(offset.x) < lineW * 4 && Math.abs(offset.y) < lineW * 4) {
  ...
  /** 导入 */
  static import(box: MarkCanvas, data: MarkObjectJSON): MarkLine_DObject {
