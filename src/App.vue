...
import { html2canvasToImgDownload } from "./plugin/MarkCanvas/src/utils/utils";
...
function exportImg() {
  console.log("mark>>>>", mark.value.view);
  html2canvasToImgDownload(mark.value.view, { scale: 1 });
  // mark.value?.exportImage(function(data:string){
...
<template>
  <div class="page">
    <div>
      <b>画布操作</b>： 1.双击鼠标恢复大小;&nbsp;&nbsp;&nbsp;2.Ctrl +
      滚轮缩放;&nbsp;&nbsp;&nbsp;3.按住空格拖动画布<br />
      <b>多边形绘制</b>： 1.右键删除最后一个点;&nbsp;&nbsp;&nbsp;2.点击第一个点或者按回车
      完成绘制<br />
      <b>标注对象</b>：<br />
    ...
    <div class="tools_bar">
      ...
      <button
        :class="{ active: selectStatus }"
        @click="mark?.setSelectMode(!selectStatus)"
      >
        <!-- 选择 -->
        <svg
          t="1689228737226"
      ...
      <button :class="{ active: drawType == 'line' }" @click="setDrawType('line')">
        实线
      </button>
      <button :class="{ active: drawType == 'line_d' }" @click="setDrawType('line_d')">
        虚线
      </button>
      <div class="b"></div>
